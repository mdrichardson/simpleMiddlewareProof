...
var UniversalBot = (function (_super) {
    ...
    UniversalBot.prototype.getStorageData = function (storageCtx, done, error) {
        try {
            const data = JSON.stringify({ conversation: storageCtx.conversationId });
            console.log(`**** STATESTORE Getting Data | UniversalBot getStorageData() | ${ data }`);
        } catch (err) { 
            console.log(`**** [ERROR] STATESTORE Getting Data | UniversalBot getStorageData() | ${ JSON.stringify(storageCtx) }`); 
        }
        var _this = this;
        this.tryCatch(function () {
            ...
            storage.getData(storageCtx, function (err, data) {
                if (!err) {
                    ...
                    try {
                        const data = JSON.stringify({ conversation: storageCtx.conversationId });
                        console.log(`**** STATESTORE Got Data | UniversalBot getStorageData() | ${ data }`);
                    } catch (err) { 
                        console.log(`**** [ERROR] STATESTORE Got Data | UniversalBot getStorageData() | ${ JSON.stringify(storageCtx) }`); 
                    }
                }
                else if (error) {
                    error(err);
    ...
    UniversalBot.prototype.saveStorageData = function (storageCtx, data, done, error) {
        try {
            const data = JSON.stringify({ conversation: storageCtx.conversationId });
            console.log(`**** STATESTORE Saving Data | UniversalBot saveStoragedata() | ${ data }`);
        } catch (err) { 
            console.log(`**** [ERROR] STATESTORE Saving Data | UniversalBot getStorageData() | ${ JSON.stringify(storageCtx) }`); 
        }
        var _this = this;
        this.tryCatch(function () {
            ...
            storage.saveData(storageCtx, data, function (err) {
                if (!err) {
                    if (done) {
                        ...
                        try {
                            const data = JSON.stringify({ conversation: storageCtx.conversationId });
                            console.log(`**** STATESTORE Saved Data | UniversalBot saveStoragedata() | ${ data }`);
                        } catch (err) { 
                            console.log(`**** [ERROR] STATESTORE Saved Data | UniversalBot getStorageData() | ${ JSON.stringify(storageCtx) }`); 
                        }
                    }
                }
                else if (error) {
