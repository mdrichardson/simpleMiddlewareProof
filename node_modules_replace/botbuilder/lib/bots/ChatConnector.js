...
var pjson = require('../../package.json.js');
...
var ChatConnector = (function () {
                ...
                req.on('end', function () {
                    try {
                        ...
                        if (req.body.type === 'message') {
                            console.log('ChatConnector Received Message | ChatConnector > listen()');
                        }
                    }
                    catch (err) {
    ...
    ChatConnector.prototype.send = function (messages, done) {
        ...
        async.forEachOfSeries(messages, function (msg, idx, cb) {
            try {
                ...
                console.log('ChatConnector Done Sending Message | ChatConnector > send()');
            }
    ...
    ChatConnector.prototype.postMessage = function (msg, lastMsg, cb, method) {
        ...
        if (msg.type === 'message') {
            console.log('ChatConnector Sending Message | ChatConnector > postMessage()');
        }
        this.authenticatedRequest(options, function (err, response, body) {
            if (!err) {
                if (msg.type === 'message') {
                    console.log('ChatConnector Received Successful Response After Send | ChatConnector > postMessage() > authenticatedRequest()');
                }
                if (body && body.id) {
