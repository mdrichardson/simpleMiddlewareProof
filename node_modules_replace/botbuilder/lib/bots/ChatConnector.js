...
var ChatConnector = (function () {
    ...
    ChatConnector.prototype.send = function (messages, done) {
        var _this = this;
        var addresses = [];
        const original_message = { messages: JSON.parse(JSON.stringify(messages)) };
        async.forEachOfSeries(messages, function (msg, idx, cb) {
            try {
                ...
                try {
                    const activity = msg;
                    if (activity.type === 'message') {
                        const data = JSON.stringify({ conversation: original_message.messages[0].address.conversation });
    ...
    ChatConnector.prototype.postMessage = function (msg, lastMsg, cb, method) {
        ...
        try {
            const activity = msg;
            if (activity.type === 'message') {
                const data = JSON.stringify({ conversation: address.conversation });
