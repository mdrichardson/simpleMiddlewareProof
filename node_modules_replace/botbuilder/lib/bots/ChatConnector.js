...
var ChatConnector = (function () {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (activity.type === 'message') {
                                const data = JSON.stringify({ conversation: activity.conversation, id: activity.id, user: activity.from });
                                console.log(`**** CHATCONNECTOR Received Message | ChatConnector listen() | ${ data }`);
                            }
                        } catch (err) { 
                            console.log(`**** [ERROR] CHATCONNECTOR Received Message | ChatConnector listen() | ${ JSON.stringify(req) }`); 
                        }
                    }
                    catch (err) {
        ...
        async.forEachOfSeries(messages, function (msg, idx, cb) {
            try {
                ...
                try {
                    const activity = msg;
                    if (activity.type === 'message') {
                        const data = JSON.stringify({ user: activity.recipient });
                        console.log(`**** CHATCONNECTOR Done Sending Message | ChatConnector end() | ${ data }`);
                    }
                } catch (err) { 
                    console.log(`**** [ERROR] CHATCONNECTOR Done Sending Message | ChatConnector end() | ${ JSON.stringify(msg) }`); 
                }
            }
    ...
    ChatConnector.prototype.getData = function (context, callback) {
        var _this = this;
        try {
            console.log(StateApiDreprecatedMessage);
    ...
    ChatConnector.prototype.postMessage = function (msg, lastMsg, cb, method) {
        ...
        try {
            const activity = msg;
            if (activity.type === 'message') {
                const data = JSON.stringify({ conversation: address.conversation, user: activity.recipient });
                console.log(`**** CHATCONNECTOR Sending Message | ChatConnector postMessage() | ${ data }`);
            }
        } catch (err) { 
            console.log(`**** [ERROR] CHATCONNECTOR Sending Message | ChatConnector postMessage() | ${ JSON.stringify(msg) }`); 
        }
        this.authenticatedRequest(options, function (err, response, body) {
            if (!err) {
                if (body && body.id) {
